...
export default function Header({ locale }: HeaderProps) {
  ...
  return (
    <>
      ...
      >
        <div
          className="pointer-events-auto flex items-center justify-between box-border w-[94%] max-w-[1320px] h-16 sm:h-16 md:h-20 rounded-sm bg-gradient-to-b from-[#F3EFE5]/95 to-[#ECE6D8]/95 backdrop-blur-md border-2 border-black shadow-[0_8px_30px_rgba(0,0,0,0.25)] px-3 sm:px-6 lg:px-8"
        >
          {/* Left Brand Lockup */}
          <Link
            href={`/${locale}`}
          ...
          >
            <div className="relative h-11 sm:h-14 md:h-16 lg:h-[68px] w-36 sm:w-52 md:w-60 lg:w-68 shrink-0 flex items-center">
              <Image
                src="/mmainlogot1.png"
                alt={isRtl ? SITE_CONFIG.companyName.ar : SITE_CONFIG.companyName.en}
                fill
                priority
                sizes="(max-width: 640px) 144px, (max-width: 1024px) 240px, 280px"
          ...
          <div className="flex items-center space-x-2 sm:space-x-5 rtl:space-x-reverse">
            ...
            <Link
              href={targetPath}
              className="font-tech text-xs tracking-wider uppercase text-[#3A3A34] hover:text-black border border-transparent hover:border-[#D4CBB8] px-2 py-2 min-h-[44px] flex items-center justify-center transition-all touch-feedback select-none font-bold"
            >
              {targetLocale === "ar" ? "العربية" : "EN"}
            </Link>

            {/* Dark Live RFQ Cart Button: RFQ [02] */}
            <button
              onClick={openMiniCart}
              className={`group relative inline-flex items-center space-x-1.5 rtl:space-x-reverse font-tech text-xs tracking-wider uppercase px-2.5 sm:px-3.5 py-2 min-h-[44px] bg-[#111418] hover:bg-[#1C2026] text-[#F3EFE5] border transition-all duration-300 active:translate-y-0.5 select-none touch-feedback ${
                pulseTrigger
            ...
            >
              <span className={`w-1.5 h-1.5 rounded-full transition-all ${pulseTrigger ? "bg-[#C8A94A] animate-ping" : "bg-[#C8A94A]"}`} />
              <span className="font-bold">{isRtl ? "طلب الأسعار" : "RFQ"}</span>
              <span
                className={`font-bold font-mono text-xs transition-colors ${
            ...
            <button
              ...
              className="lg:hidden font-tech text-xs tracking-widest text-[#3A3A34] border border-[#D4CBB8] px-3 py-2 min-h-[44px] min-w-[44px] flex items-center justify-center uppercase hover:border-[#C8A94A] bg-[#ECE6D8]/80 transition-colors shadow-sm select-none touch-feedback font-bold"

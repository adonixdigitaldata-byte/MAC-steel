...
export default function ContactPage() {
  ...
  return (
    <main className="min-h-screen bg-world-bone text-carbon w-full max-w-full overflow-hidden">
      ...
      <Section world="bone">
        <Container>
          <div id="rfq-form" className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12 items-start pt-4 sm:pt-8 scroll-mt-28">
            ...
            <FadeReveal className="lg:col-span-5 space-y-6 border border-bone-border bg-bone-surface p-6 sm:p-8 hover:border-accent-copper/60 hover:-translate-y-1.5 hover:shadow-[0_16px_32px_rgba(0,0,0,0.08)] transition-all duration-300 rounded-sm" y={24} duration={750}>
              <div className="flex items-center space-x-3 rtl:space-x-reverse pb-4 border-b border-bone-border">
                <NumberBadge number="05" world="bone" />
                <TechnicalLabel variant="copper">SPECIFICATION OFFICE</TechnicalLabel>
              </div>

              <DisplayM className="text-carbon text-2xl uppercase">
                {isRtl ? "قنوات التواصل المباشرة" : "ENGINEERING SUPPORT DIRECT"}
              </DisplayM>

              <BodyText className="text-accent-mineral text-xs sm:text-sm leading-relaxed">
                {isRtl
              ...
              <div className="space-y-4 font-tech text-xs border-t border-bone-border pt-6 text-accent-mineral">
                <div className="group/item">
                  ...
                  <a
                    ...
                    className="font-bold text-carbon block hover:text-accent-copper transition-colors"
                  >
                ...
                <div>
                  ...
                  <span className="font-bold text-carbon block">{SITE_CONFIG.location[locale]}</span>
                </div>
            ...
            <FadeReveal className="lg:col-span-7 border border-bone-border bg-bone-surface p-6 sm:p-10 space-y-6 hover:border-bone-border/90 hover:shadow-[0_16px_32px_rgba(0,0,0,0.08)] transition-all duration-300 rounded-sm" delay={120} y={24} duration={750}>
              ...
              {formSubmitted ? (
                <div className="p-8 border border-accent-copper bg-world-bone text-center space-y-4 my-8">
                  <TechnicalLabel variant="copper">DOCUMENT LOGGED</TechnicalLabel>
                  <DisplayM className="text-carbon">
                    {isRtl ? "تم استلام الطلب الفني" : "INQUIRY LOGGED SUCCESSFULLY"}
                  </DisplayM>
                  <BodyText className="text-accent-mineral text-sm">
                    {isRtl
                  ...
                  <button
                    ...
                    className="font-tech text-xs text-accent-copper underline uppercase pt-4 block mx-auto hover:text-carbon transition-colors"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1.5 font-tech text-xs">
                      <label className="block tracking-wider uppercase text-accent-mineral font-medium">
                        {isRtl ? "قطاع الاستخدام *" : "APPLICATION SECTOR *"}
                      </label>
                      <select
                        value={formData.sector}
                        onChange={(e) => setFormData({ ...formData, sector: e.target.value })}
                        className="w-full bg-world-bone border border-bone-border p-3 text-carbon font-tech text-xs focus:outline-none focus:border-carbon transition-colors uppercase"
                      >
                  ...
                  <div className="pt-4 border-t border-bone-border">
                    <Button type="submit" locale={locale} variant="primary" world="bone" className="w-full">
      ...
      <TechnicalDivider world="bone" label="END OF INQUIRY" documentId="DOC-RFQ-END-2026" />
    </main>
  );
}
